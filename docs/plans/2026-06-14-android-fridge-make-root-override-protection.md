# Android Fridge Make Root Override Protection
...
3. Require exact protected-root, override-semantics, rooted baseline, and
   rooted lint/test/build contracts in the dependency-free checker.
...
- Do not change fridge runtime behavior, storage handling, permissions,
...
- Do not claim emulator, device, or production verification.
...
- Local, external-directory, and hostile command-line `ROOT` `make check`
  ...
  checkout.
- No Android SDK was configured, so lint, test, and assembly truthfully
  reported their designed skips in all three contexts; no SDK-backed compile,
  emulator, device, or production result is claimed.
- All ten focused mutations were rejected: missing `override`, `CURDIR`,
  ...
  assignment, unrooted baseline, one unrooted Gradle gate, removed test task,
